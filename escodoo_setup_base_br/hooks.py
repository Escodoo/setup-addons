...
def _update_partners(env):
    ...
    First updates partners without a parent_id, then updates partners with a
    parent_id.
...
def post_init_hook(cr, registry):
    ...
    env["ir.config_parameter"].set_param(
        "pwa.manifest.name", f"Odoo Online {db_name_uppercase}"
    )
    env["ir.config_parameter"].set_param(
        "pwa.manifest.short_name", f"Odoo {db_name_uppercase}"
    )
